...
% From the stable manifold, we obtain the estimate 
% \begin{equation}\label{eq:U1decayest}
% \|U^1_M(n)\|_{X_{M,e}} \leq C r_{M,e}^{-|n|}.
% \end{equation}

% The variational equation is the linearization of \cref{eq:dynEqM} about the homoclinic orbit solution $U^1_M(n)$, given by
% \begin{equation}\label{eq:vareq}
% W(n+1) = DF_M(U_M^1(n)) W(n).
% \end{equation}
% Since the tangent spaces of $W_M^s(0)$ and $W_M^u(0)$ have a one-dimensional intersection by \cref{hyp:breather}, it follows that 
% $\dot{U}_M^1(n) = (\dot{u}_M^1(n), \dot{\tilde{u}}_M^1(n)) = (\dot{u}_M^1(n), \dot{u}_M^1(n-1))$ 
% is the unique, bounded solution to \cref{eq:vareq}, up to scalar multiples. (The fact that it is a solution can be verified by differentiating \cref{eq:DKGapprox} with respect to $t$). We can thus decompose the tangent spaces to $W_M^u(0)$ and $W_M^s(0)$ at $U_M^1(0)$ as
% \begin{equation}\label{eq:TWdecomp}
% T_{U_M^1(0)}W^u(0) = \R \dot{U}_M^1(0) \oplus Y_M^-, \qquad  
% T_{U_M^1(0)}W^s(0) = \R \dot{U}_M^1(0) \oplus Y_M^+,
% \end{equation}
% where $\dim Y_M^- = \dim Y_M^+ = 2M$. In addition, the adjoint variational equation
% \begin{equation}\label{eq:adjvareq}
% Z(n) = DF_M(U_M^1(n))^* Z(n+1)
% \end{equation}
% has a unique bounded solution, given by
% \begin{equation}\label{eq:Z1}
% Z_M^1(n) = (-\dot{u}_M^1(n-1), \dot{u}_M^1(n))^T,
% \end{equation}
% and $Z_M^1(0) \perp \R \dot{U}_M^1(0) \oplus Y_M^- \oplus Y_M^+$ by \cite{Parker2020}*{Lemma 1}. We can thus decompose $X_M$ as
% \begin{equation}\label{eq:Xdecomp}
% X_M = \R \partial_t U_1(0) \oplus Y_M^- \oplus Y_M^+ \oplus \R Z_M^1(0).
% \end{equation}

...
\title{Revisiting Multi-breathers in the discrete Klein-Gordon equation: A Spatial Dynamics Approach}
%\runningtitle{Revisiting Multi-breathers in the discrete Klein-Gordon equation}
...
\end{aligned}
\end{equation}
{\bf Ross: isn't this the first appearance of $\omega_0$? Clarify?}
...
compare with the definition of $y_n$ before and the formula above.}
...
{\bf We have used $M$ previously as the maximum Fourier coefficient}
...
So that each of them will ``spring up'' an individual breather?}
...
of the lattice and of the spatial discretization, and have merged into a single band by this value of $d$. \cref{fig:singlec} shows the exponential decay of the breather with increasing $n$; \cref{fig:singled} shows the relative error of this decay rate compared to $r_M$, which is defined in \cref{eq:defrM}. The exponential decay of the Fourier frequencies (in $t$) of the primary breather is shown in \cref{fig:freqdecay}. As expected, since the breather is even in $t$, only even frequencies are represented in the frequency spectrum. Furthermore, the scaled amplitudes for all frequencies $k\geq 20$ are below $10^{-10}$, suggesting that the finite dimensional approximation is reasonable.
...
\begin{figure}
	...
	\end{subfigure}
	\end{center}
	\caption{(a) Initial condition $u_n(0)$, and (b) Floquet spectrum, for primary breather for $d = 0.25$. (c) Semilog plot showing exponential decay rate of $L^2$ norm of breather solution with increasing $n$ for varying $d$. 
	{\bf Ross: just to clarify: the norm is over one period? (for 
	different sites). Right? Also can you clarify panel (d) ---
	decay rate of what exactly??}
	(d) Relative error in exponential decay rate. }
	\label{fig:single}
\end{figure}
...
\begin{figure}
	...
	I mean, or else I may need to explain.}}
	\label{fig:double}
\end{figure}
...
\begin{figure}
	...
	Semilog plot of the relative error of interaction eigenmode computation vs. $d$ for out-of-phase double breathers (c) and in-phase double breathers (d) for breather distance $N_1 = 4,5,6,7$. {\bf shouldn't it be $N_1$ too in upper panels?}}
	\label{fig:eigerror}
\end{figure}
...
\end{equation}
{\bf Ross: please check if the $v_n^2$ change is OK with you?}
...
\begin{figure}
	...
	\end{subfigure}
	\end{center}
	\caption{Colormap showing the evolution in $t$ of $|u_n|$ (left), energy density vs. time for central sites of breather (center and right). Labels $n$ of sites correspond to those in left panel. Separation distance $N_1 = 3$ (top), $N_1 = 4$ (bottom).
